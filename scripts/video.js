document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const openModalBtn = document.getElementById("update-session-btn"); // Bouton d'ouverture
        const closeModal = document.getElementById("close-update-modal"); // Bouton de fermeture
        const submitButton = document.getElementById("submitUpdateSeance"); // Bouton de mise à jour
        
        // Quand l'utilisateur clique sur le bouton "Modifier"
        document.addEventListener('click', async (event) => {
            if (event.target.classList.contains('modify-session-btn')) {
                const sessionId = event.target.dataset.sessionId; // Récupère l'ID de la séance
        
                try {
                    // Récupérer les détails de la séance
                    const response = await axios.get(`http://localhost:1234/video/getSeance/${sessionId}`);
                    const sessionData = response.data;
        
                    // Pré-remplir les champs du formulaire avec les données de la séance
                    document.getElementById("update-session-name").value = sessionData.titre;
                    document.getElementById("update-session-lieu").value = sessionData.lieu;
                    document.getElementById("update-session-sport").value = sessionData.id_sport;
                    document.getElementById("update-session-taille").value = sessionData.nombrePlaces;
                    document.getElementById("update-session-dateDebut").value = sessionData.dateDebut;
                    document.getElementById("update-session-dateFin").value = sessionData.dateFin;
        
                    // Afficher la modale
                    modal.style.display = "block";
        
                    // Définir l'action du bouton de soumission pour la mise à jour
                    submitButton.onclick = async function (e) {
                        e.preventDefault();
                        await updateSeance(sessionId);
                    };
                } catch (error) {
        ...
        // Fonction pour mettre à jour la séance
        async function updateSeance(sessionId) {
            const sessionData = {
                titre: document.getElementById("update-session-name").value,
                lieu: document.getElementById("update-session-lieu").value,
                id_sport: document.getElementById("update-session-sport").value,
            ...
            try {
                const response = await axios.put(`http://localhost:1234/video/updateSeance/${sessionId}`, sessionData, {
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': localStorage.getItem('token') // Ajoutez l'authentification si nécessaire
                    }
                });
        
                alert("Séance mise à jour avec succès !");
                modal.style.display = "none"; // Fermer la modale après la mise à jour
            } catch (error) {
